...
// ══ 景點挑戰：隨機 5 題（3 景點知識 + 2 英文單字）計分，像問答 ══════
let spotQuizSet=[], spotQuizPos=0, spotQuizPicks={}, spotQuizId=null;
...
function _shuffleOpts(q){
  const order=_qshuffle(q.opts.map((_,i)=>i));
  return { q:q.q, tag:q.tag, opts:order.map(i=>q.opts[i]), ans:order.indexOf(q.ans) };
}
...
const KNOWLEDGE=[
 ...
 {emoji:'🏗️',title:'摩天大樓',sub:'為什麼紐約有這麼多高樓？',
  body:`紐約是「摩天大樓」的故鄉！「Skyscraper」這個字原本是指帆船上最高的桅杆，後來用來形容高得好像要刮到天空的大樓。\n\n曼哈頓是一座島，土地有限，沒辦法往兩邊蓋，所以大家就往上蓋，越蓋越高！\n\n最有名的帝國大廈只花了 410 天就建好，晚上頂端的燈還會根據節日變換顏色。新的觀景台像 Summit One Vanderbilt，有會反射的鏡子房間，還有伸出大樓外的透明玻璃地板，站上去能看到腳下的紐約，超刺激！`},
...
const FLASHCARDS=[
 ...
 {emoji:'👋',en:'Goodbye',phon:'/ˌɡʊdˈbaɪ/',zh:'再見'},
 {emoji:'🙇',en:'Sorry',phon:'/ˈsɑːri/',zh:'對不起'},
 {emoji:'🙋',en:'Excuse me',phon:'/ɪkˈskjuːz miː/',zh:'請問／借過'},
 {emoji:'🚪',en:'Exit',phon:'/ˈeɡzɪt/',zh:'出口'},
...
const PIN_NAME={yankee:'洋基球場',liberty:'自由女神',brooklyn:'布魯克林大橋',dream:'美國夢',amnh:'自然史博物館',met:'大都會',rock:'洛克菲勒',broadway:'獅子王',edge:'Summit',highline:'高線公園',village:'格林威治村','911':'911紀念館'};
...
function buildMapStatic(){
  ...
  mv.innerHTML=h;
  mapBuilt=true;
  // 注意：州/行政區的點擊改由 initMapGestures 的 handleMapTap 處理（避免 pointer capture 吃掉 click）
}
...
function applyMap(){
  ...
  pl.innerHTML=h;
  // 注意：pin 的點擊由 initMapGestures 的 handleMapTap 處理（pointer capture 會吃掉合成 click）
  // zoom level label
...
function initMapGestures(){
  ...
  let pts=new Map(), startDist=0, startScale=1, startMid=null, panStart=null, moved=false, tapInfo=null;
  function svgPt(e){
  ...
  svg.addEventListener('pointerdown',e=>{
    ...
    if(pts.size===1){ panStart={...[...pts.values()][0], tx:mapTx, ty:mapTy}; tapInfo={x:e.clientX,y:e.clientY,id:e.pointerId,t:Date.now()}; }
    else if(pts.size===2){
      tapInfo=null;
  ...
  function handleMapTap(cx,cy){
    ...
    const pin=t.closest('.pin-tap');
    if(pin){ const s=pin.getAttribute('data-spot'); if(s==='__nyc__'){ zoomPreset('nyc'); } else { openDetail(s); } return; }
    const boro=t.closest('.boro-shape');
    if(boro){ showBoro(boro.getAttribute('data-boro')); return; }
    const st=t.closest('.us-state');
    if(st && st.getAttribute('data-state')==='New Jersey'){ showBoro('nj'); }
  }
  function up(e){
    let tap=null;
    if(tapInfo && e.pointerId===tapInfo.id && pts.size===1){
      const dx=e.clientX-tapInfo.x, dy=e.clientY-tapInfo.y;
      if(Math.hypot(dx,dy)<10 && Date.now()-tapInfo.t<600) tap={x:e.clientX,y:e.clientY};
    }
    pts.delete(e.pointerId); if(pts.size<2)startMid=null; if(pts.size===0){panStart=null; tapInfo=null;}
    if(tap) handleMapTap(tap.x,tap.y);
  }
