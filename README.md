...
- **V1.16.0** — 小小探險家的機器人可以自己組合了！換顏色、眼睛、嘴巴、天線、臉型、側邊，或按「隨機組合」。由 DiceBear（Bottts）產生器離線即時繪製。
- **V1.15.0** — 小小探險家的頭像改成可愛的機器人（24 種，可隨機換一隻），由開源頭像庫 DiceBear（Bottts）提供，取代原本手繪的人臉捏臉。
- **V1.14.2** — 行程頁對齊優化：每段的標題與圖示，現在和下方每一列的圖示、內容對齊在同一條軸線上，視覺更整齊。
- **V1.14.1** — 修正行程頁在系統深色模式下仍顯示黑底的問題：行程頁現固定為淺色，不再跟隨系統深色模式。
